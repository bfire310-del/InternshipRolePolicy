...
public class LoginQueryHandler(IUsersRepository usersRepository,
    IJwtService jwtService) : IRequestHandler<LoginQuery, Result<(string, string, string, string)>>
{
    public async Task<Result<(string, string, string, string)>> Handle(LoginQuery request, CancellationToken cancellationToken)
    {
        ...
        if (user.IsFailed)
            return Result.Failure<(string, string, string, string)>(user.Error);

        var token = await jwtService.GenerateToken(user.Value.UserId, user.Value.Role.Code);

        return Result.Success((token.Value, user.Value.Role.Code, user.Value.FirstName, user.Value.LastName));
    }
}
